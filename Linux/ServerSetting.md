...
## 7. tmux
- 일반적으로 ssh로 접속해서 사용하는 경우 ssh 접속이 끊어지면 ssh 서버도 해당 프로세스를 중단한다.
- ssh 클라이언트가 접속을 끊더라도 서버에서 해당 작업을 계속하도록 하기 위해 사용한다.
######
- tmux 를 실행하면 하나의 윈도우를 가지는 새로운 세션을 만들고 이것을 화면에 출력한다.
- 화면 아래에 상태 표시줄에 현재 세션 정보가 표시되며 이는 interactive commands를 입력하는데 사용된다.
- 세션은 tmux가 관리하는 가상의 터미널 집합이다. 각각의 세션은 그것과 연결된 하나 또는 2개 이상의 윈도우를 갖고 있따.
- 윈도우는 모니터 화면 전체를 차지하고 있고, 이 윈도우는 몇개의 패널로 나눌 수 있으며 각각의 패널은 분리된 가성의 터미널 이다.

### 7-1. Session
- tmux 는 tmux 서버, tmux 클라이언트로 나눌 수 있다.
- 이때 서버에서 실행되는 프로세스를 세션이라고 한다.
- 세션은 tmux가 관리하는 가성의 터미널로 1개 이상의 클라이언트가 접속할 수 있다.
...
######
    ...
    // 세션 다시 접속
    % tmux attach-session -t 세션이름(세션번호)
